<?php
/**
 * 스마트알람 (Smart-Alarm for Gnuboard4)
 *
 * Copyright (c) 2011 Choi Jae-Young <www.miwit.com>
 *
 * 저작권 안내
 * - 저작권자는 이 프로그램을 사용하므로서 발생하는 모든 문제에 대하여 책임을 지지 않습니다. 
 * - 이 프로그램을 어떠한 형태로든 재배포 및 공개하는 것을 허락하지 않습니다.
 * - 이 저작권 표시사항을 저작권자를 제외한 그 누구도 수정할 수 없습니다.
 */

include_once("_common.php");
include_once("_config.php");

if (!$is_member) exit;

$moa_config = sql_fetch("select * from $mw_moa_config_table where mb_id = '$member[mb_id]' ");
if (!$moa_config)
    sql_query("insert into $mw_moa_config_table set mb_id = '$member[mb_id]' ");

if ($arm) { // 알람
    if (strstr($moa_config[cf_reject_board], "$bo_table,")) {
        $moa_config[cf_reject_board] = str_replace("$bo_table,", "", $moa_config[cf_reject_board]);
    }
} else { // 해제
    if (!strstr($moa_config[cf_reject_board], "$bo_table,")) {
        $moa_config[cf_reject_board] .= "$bo_table,";
    }
}
sql_query("update $mw_moa_config_table set cf_reject_board = '$moa_config[cf_reject_board]' where mb_id = '$member[mb_id]'");

