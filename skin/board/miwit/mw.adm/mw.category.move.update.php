...
$wr_id_list = array();
$sql = " select wr_id from {$write_table} where ca_name = '{$ca_name}' and wr_is_comment = 0 ";
$qry = sql_query($sql);
while ($row = sql_fetch_array($qry)) {
    $wr_id_list[] = $row['wr_id'];
}
$wr_id_list = implode(",", $wr_id_list);
...
$msg = "{$ca_name} 분류의 게시물을 선택한 게시판으로 $act 하였습니다.";
